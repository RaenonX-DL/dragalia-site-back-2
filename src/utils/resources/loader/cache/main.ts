...
import {generateCacheBody, isCacheExpired} from '../../../cache/func';
...
const setCache = async <K extends CacheKey>(
  ...
  if (!cache[CacheKey.UNIT_INFO].data.size || isCacheExpired(cache[CacheKey.UNIT_INFO], epoch)) {
...
export const getCache = async <K extends CacheKey>(
  ...
  if (isCacheExpired(cache[cacheKey], currentEpoch)) {
